 ...
 * Tree View — https://app.subframe.com/51ef4e586a6b/library?component=Tree+View_4ed46422-ecc3-41e8-8787-e55ee10cdc75
 */
    ...
    <Accordion
      ...
      trigger={
        <div className="flex w-full items-center gap-2 rounded-md px-3 py-2 group-hover/c841484c:bg-neutral-50">
          {icon ? (
            <SubframeCore.IconWrapper className="text-body font-body text-light-background">
              {icon}
            </SubframeCore.IconWrapper>
          ) : null}
          {label ? (
            <span className="line-clamp-1 grow shrink-0 basis-0 text-body font-body text-light-background">
              {label}
            </span>
          ) : null}
          <Accordion.Chevron />
  ...
  return (
    ...
    >
      ...
      {label ? (
        <span
          className={SubframeUtils.twClassNames(
            "line-clamp-1 grow shrink-0 basis-0 text-body font-body text-light-background",
            { "text-brand-700": selected }
          )}
        >
          {label}
