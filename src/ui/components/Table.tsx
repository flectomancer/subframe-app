...
/*
 * Documentation:
 * Table — https://app.subframe.com/51ef4e586a6b/library?component=Table_142dfde7-d0cc-48a1-a04c-a08ab2252633
 */
  ...
  function HeaderCell(
    ...
    return (
      <th {...otherProps}>
        ...
        >
          {children ? (
            <span className="whitespace-nowrap text-caption-bold font-caption-bold text-light-primary">
              {children}
            </span>
          ) : null}
          {icon ? (
            <SubframeCore.IconWrapper className="text-caption font-caption text-light-primary">
              {icon}
...
const Row = React.forwardRef<HTMLTableRowElement, RowProps>(function Row(
  ...
  return (
    <tr
      className={SubframeUtils.twClassNames(
        "group/5d119f8d border-t border-solid border-neutral-50",
...
const TableRoot = React.forwardRef<HTMLTableElement, TableRootProps>(
  function TableRoot(
    ...
    return (
      ...
      >
        <thead>{header}</thead>
        <tbody className="border-b border-solid border-neutral-50">
          {children}
        </tbody>
      </table>
    );
